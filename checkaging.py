...
def check_unprocessed_orders():
    ...
    try:
        gc_client = get_gspread_client(SHEET_KEY_DATA)
        ...
        sh_main.batch_update({
            "requests": [
                cell_format_request(ws_unprocessed.id, 0, 1, 0, len(unprocessed_headers), {
                    "backgroundColor": make_color("#E11D48"),
    ...
    caption += f"🔗 <b>Link danh sách đơn CHƯA GÁN để AM push bưu cục:</b> <a href=\"{sheet_link}\"><b>Xem chi tiết tại đây</b></a>\n\n"
    ...
    if unassigned_2days_plus_grand > 0:
        ...
        caption += f"  • 🚫 <b>Chưa gán giao (Bị dồn):</b> <b>{len(unassigned_orders)}</b> đơn (Shipper chưa nhận đi chuyến nào)\n"
    ...
    caption += f"🏆 <b>DANH SÁCH AM CẦN PUSH GẤP (Xếp theo đơn chưa gán giảm dần):</b>\n"
