...
docx_out = r"C:\Users\lap4all\Downloads\Ban_Do_Quy_Hoach_NTB_Final_Drawn_Maps.docx"
pdf_out = r"C:\Users\lap4all\Downloads\Ban_Do_Quy_Hoach_Mang_Luoi_NTB_2026.pdf"

# Helper to decorate map with legend box
def decorate_map(src_path, dst_name, has_new=True, has_move=True):
    ...
    box_w = int(w * 0.28)
    box_h = int(h * 0.04) + len(items) * int(h * 0.045)
    box_x1 = w - int(w * 0.03) - box_w
    box_y1 = int(h * 0.03)
    box_x2 = w - int(w * 0.03)
    box_y2 = box_y1 + box_h
    
    draw.rectangle([box_x1, box_y1, box_x2, box_y2], fill=(255, 255, 255, 235), outline=(180, 180, 180, 255), width=2)
    
    y_c = box_y1 + int(h * 0.015)
    for sym, color, txt in items:
        draw.text((box_x1 + int(w * 0.015), y_c - int(h * 0.005)), sym, fill=color, font=font_leg)
        draw.text((box_x1 + int(w * 0.045), y_c), txt, fill=(15, 23, 42, 255), font=font_leg)
        y_c += int(h * 0.045)
...
r_sub = p_sub.add_run("BẢN ĐỒ KHOANH VÙNG RANH GIỚI & CHÚ GIẢI BƯU CỤC MỚI / BƯU CỤC HIỆN HỮU")
r_sub.bold = True
...
# Sections structure matching Telegram example format
sections = [
    {
        ...
        "items": [
            ...
            },
            {
                ...
                "img": decorate_map(os.path.join(web_dir, "map_whatif_ninh_hoa.png"), "dec_ninh_hoa.png"),
    ...
    },
    {
        ...
        "items": [
            {
                "title": "2.1. Bản đồ Phân vùng Tách Bưu cục Lạc Xuân - Huyện Đơn Dương",
                "img": os.path.join(artifact_dir, "media__1785896953199.png"),
            ...
            {
                "title": "2.2. Bản đồ Quy hoạch Mở mới Bưu cục B'Lao Mới - TP. Bảo Lộc",
                "img": decorate_map(os.path.join(web_dir, "map_whatif_bao_loc.png"), "dec_bao_loc.png"),
            ...
            {
                "title": "2.3. Bản đồ Quy hoạch TP. Đà Lạt - BC Xuân Hương 2 Mở Mới",
                "img": decorate_map(os.path.join(web_dir, "map_whatif_da_lat.png"), "dec_da_lat.png"),
                "desc": "Khu Vực TP. Đà Lạt\n- Mở Mới BC Xuân Hương 2 (đặt tại Phường 10, TP. Đà Lạt) cover Phường 3 & Phường 10 để chia tải cho BC Xuân Hương cũ (>2.850 đơn/ngày)."
            }
        ]
    },
    {
        ...
        "items": [
            {
                ...
                "img": decorate_map(os.path.join(web_dir, "map_whatif_phan_rang.png"), "dec_phan_rang.png"),
        ...
        "items": [
            {
                ...
                "img": decorate_map(os.path.join(web_dir, "map_whatif_nam_thanh.png"), "dec_nam_thanh.png"),
            ...
            },
            {
                ...
                "img": decorate_map(os.path.join(web_dir, "map_whatif_phan_thiet.png"), "dec_phan_thiet.png"),
        ...
        "items": [
            {
                ...
                "img": decorate_map(os.path.join(web_dir, "map_whatif_gia_nghia.png"), "dec_gia_nghia.png"),
...
for sec in sections:
    ...
    for item in sec["items"]:
        ...
        if os.path.exists(item["img"]):
            ...
            p_img.add_run().add_picture(item["img"], width=Inches(6.8))
...
doc.save(docx_out)
print(f"Successfully created Final Drawn Maps DOCX at: {docx_out}")
...
doc_com.Close()
word.Quit()
print(f"Successfully generated FINAL DRAWN MAPS PDF at: {pdf_out}")
