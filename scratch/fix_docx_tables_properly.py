...
# Find paragraphs where IV starts
iv_idx = None
...
# Delete all paragraphs from iv_idx to the end
if iv_idx is not None:
    for p in list(doc.paragraphs[iv_idx:]):
        p._element.getparent().remove(p._element)

# Remove ALL tables in the document that appear after iv_idx or duplicate tables
for tbl in list(doc.tables):
    tbl._element.getparent().remove(tbl._element)

# Now rebuild Section IV & Section V perfectly in correct order

# 1. Section IV Heading
...
# Item 2: Heading 2 -> Table 1 (Overloaded Hubs)
p = doc.add_paragraph()
...
# Item 3: Heading 3 -> Table 2 (Cross routes)
p = doc.add_paragraph()
...
# Try saving to FIXED path
try:
    doc.save(target_path)
    print(f"Successfully cleaned & saved DOCX to: {target_path}")
except Exception as e:
    print("Could not save to target_path, saving to alternative:", e)
    alt_path = r"C:\Users\lap4all\Downloads\Bao_Cao_Quy_Hoach_ MANG LUOI NTB_CLEANED.docx"
    doc.save(alt_path)
