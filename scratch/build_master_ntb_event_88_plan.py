...
add_p(doc, "TÀI LIỆU VẬN HÀNH CHUẨN CÔNG TY (FULL EXECUTIVE PLAN)", bold=True, font_size=11, align=WD_ALIGN_PARAGRAPH.CENTER, space_after=2, color_rgb=(0, 0, 0))
...
add_p(doc, "1. Chỉ tiêu Vận hành Cốt lõi (KPI Event 8.8):", bold=True, font_size=10.5, color_rgb=(0, 114, 188))
add_p(doc, "• Tỷ lệ Giao thành công (GTC): Đảm bảo duy trì tỷ lệ GTC đạt trên 65% trên toàn Vùng Nam Trung Bộ trong suốt 10 ngày Event.")
add_p(doc, "• Tỷ lệ Lấy thành công (LTC): Đảm bảo duy trì tỷ lệ LTC đạt trên 95% trong suốt kỳ Event.")
add_p(doc, "• Tỷ lệ đi làm của nhân sự: Duy trì lịch làm việc của Nhân viên Phát triển Thị trường (NVPTTT) trên mức 92% toàn Vùng trong suốt giai đoạn cao điểm từ 06/08 đến 15/08/2026.")
add_p(doc, "• Clear hàng tồn đọng: Xử lý sạch hàng tồn trên 48 giờ trước 22h hàng ngày, đặc biệt chú trọng và ưu tiên nhóm hàng có đặc tính nhỏ, nhẹ (TTS/SME).")
add_p(doc, "• Kiểm soát tỷ lệ rớt LC: Kiểm soát nghiêm ngặt tỷ lệ rớt luân chuyển đối với hàng lấy ở mức dưới 3%.")

add_p(doc, "", space_after=6)
add_p(doc, "2. Thông tin Tổng quan Vùng (Metadata Table):", bold=True, font_size=10.5, color_rgb=(0, 114, 188))
...
add_p(doc, "Sản lượng lấy toàn Vùng NTB dao động từ 48.250 đến 69.822 đơn/ngày, Sản lượng trung bình đạt 58.348 đơn/ngày. Từ ngày 06/08 đến 08/08, sản lượng tăng liên tục từ 53.120 đơn và Đỉnh điểm tăng mạnh lên 69.822 đơn (+19.66%) vào ngày 08/08.")

add_centered_picture(doc, os.path.join(charts_dir, 'ghn_combo_lay.png'), width_in_inches=6.3, caption_text="Biểu đồ Tổng quan FC Volume Lấy event 08.08 (Style GHN Corporate)")

add_ghn_heading(doc, "Chi tiết loại hàng", level=3)
add_p(doc, "• TTS và SME đóng góp khoảng 75-85% tổng sản lượng FC Lấy, đây là hai nhóm hàng cần được ưu tiên hàng đầu trong công tác bố trí nguồn lực nhân sự và CCDC.")
add_p(doc, "• Đỉnh sản lượng Lấy không xuất hiện vào ngày khởi động 06/08 mà rơi vào ngày 08/08. Cần chủ động theo dõi sát chỉ số %OPR hằng ngày, đồng thời làm việc chặt chẽ với seller đảm bảo nguồn lực tạo đơn đúng thời điểm, chuẩn bị đủ biên bản làm việc (BB), biên bản giao nhận hàng, Video có định vị, giải trình KA (nếu cần), qua đó cải thiện tình trạng gán lấy trễ và nâng % OPR đảm bảo chất lượng vận hành.")
add_p(doc, "• Bưu cục cần dự phòng thêm máy phát điện tránh trường hợp BC mất điện không thể đóng hàng làm rớt luân chuyển ảnh hưởng tỷ lệ lấp đầy thùng xe + phát sinh chi phí Linehaul hoặc điều tải nội vùng.")
...
add_centered_picture(doc, os.path.join(charts_dir, 'ghn_lay_san.png'), width_in_inches=6.2, caption_text="Biểu đồ 1: Chi tiết Sản lượng Lấy theo Sàn (Event 8.8 NTB)")

add_p(doc, "", space_after=6)

# Table 2: Volume Lấy theo Tỉnh & Tỷ trọng trung bình
add_ghn_heading(doc, "Volume Lấy theo Tỉnh và tỷ trọng trung bình từ 06-15.08", level=3)
add_p(doc, "• Nhóm trọng điểm (Dẫn đầu khối lượng): Khánh Hòa và Lâm Đồng là hai tỉnh dẫn đầu sản lượng Lấy của toàn vùng NTB. Riêng Khánh Hòa chạm mốc 24,150 đơn, Lâm Đồng 18,650 đơn trong ngày đỉnh điểm 08/08.")
add_p(doc, "• Nhìn chung sản lượng Lấy trong kỳ Event 8.8 có sự tăng trưởng nhịp nhàng, tạo điều kiện thuận lợi cho khâu chuẩn bị nhân lực và tải kết nối Linehaul không bị đứt đoạn.")
...
add_centered_picture(doc, os.path.join(charts_dir, 'ghn_lay_tinh.png'), width_in_inches=6.2, caption_text="Biểu đồ 2: Sản lượng Lấy theo Tỉnh/Quận & Tỷ trọng trung bình (Event 8.8 NTB)")
...
add_ghn_heading(doc, "Tổng quan", level=3)
add_p(doc, "Toàn vùng NTB chạm đỉnh giao vào ngày Thứ Bảy (08/08) với 97,649 đơn (tăng vọt +29.69% so với trung bình đợt). Áp lực giao duy trì ở mức cao xuyên suốt từ ngày 07/08 đến ngày 11/08.")

add_centered_picture(doc, os.path.join(charts_dir, 'ghn_combo_giao.png'), width_in_inches=6.3, caption_text="Biểu đồ Tổng quan FC Volume Giao event 08.08 (Style GHN Corporate)")

add_ghn_heading(doc, "Chi tiết nhóm hàng", level=3)
add_p(doc, "• Ngày Peak 08/08 (97,649 đơn): Hoàn toàn bị chi phối bởi Shopee. Vào ngày này, sản lượng Shopee tăng vọt lên 48,250 đơn (gần gấp đôi so với ngày thường).")
add_p(doc, "• Các ngày 09/08 - 11/08: Áp lực tổng thể vẫn duy trì ở mức rất cao do lượng đơn đổ về từ các ngày trước đó và sự giữ nhịp ổn định của sàn SME (quanh 22,000 đơn/ngày) cùng TTS (quanh 19,000 đơn/ngày).")
add_p(doc, "• Shopee-Bulky đột biến mạnh: Ngày thường nhóm này chỉ dao động quanh mức 2,500 – 4,000 đơn. Tuy nhiên, vào ngày cao điểm 08/08, Shopee-Bulky bùng nổ tăng vọt lên 12,850 đơn (gấp gần 4 lần ngày thấp điểm), trong đó nhóm nặng từ 10-15kg chiếm tới 6,200 đơn.")
add_p(doc, "• AM cần check kỹ các mã đơn có phải revert lần 2 từ GTX hay ảnh hưởng tăng mạnh từ phía người tiêu dùng nhằm có phương án ngăn chặn từ đầu tránh gánh sản lượng từ GTX khi chưa chuẩn bị đủ nguồn lực. Ngoài ra cần chủ động bố trí xe ba gác, xe van, xe tải nhỏ để giải phóng hàng CK; ưu tiên điều phối hàng CK vào 01 khung giờ (14h-16h) để giải phóng diện tích Bưu cục.")
...
add_centered_picture(doc, os.path.join(charts_dir, 'ghn_giao_san.png'), width_in_inches=6.2, caption_text="Biểu đồ 3: Chi tiết Sản lượng Giao theo Sàn (Event 8.8 NTB)")

add_p(doc, "", space_after=6)

# Table 4: Volume Giao theo Tỉnh & Tỷ trọng trung bình
add_ghn_heading(doc, "Volume Giao theo Tỉnh và tỷ trọng trung bình từ 06-15.08", level=3)
add_p(doc, "• Khánh Hòa & Lâm Đồng: Đây là hai tỉnh có sản lượng giao lớn và áp lực nặng nề nhất vùng NTB. Vào ngày lập đỉnh 08/08, Khánh Hòa bùng nổ 32,150 đơn, Lâm Đồng gánh 24,820 đơn. Nguy cơ quá tải hàng CK tại các khu vực Đà Lạt, Đức Trọng, Cam Ranh. Chuẩn bị kỹ nguồn lực NVPTTT và phương tiện tải tại các khu vực trọng điểm nhằm tăng tỷ lệ GTC đối với BC có sản lượng lớn.")
add_p(doc, "• Bình Thuận & Ninh Thuận: Hai tỉnh này có diễn biến sản lượng tương đồng và bám sát nhau (Bình Thuận peak 19,450 đơn, Ninh Thuận 11,200 đơn ngày 08/08). Các Bưu cục Phan Thiết, Ninh Hải cần đặt lên diện cảnh báo FC chuẩn bị sẵn nguồn lực ứng phó.")
add_p(doc, "• Đắk Nông: Mặc dù sản lượng đứng thứ 5 toàn vùng (đỉnh điểm 10,029 đơn ngày 08/08) nhưng là nơi tập trung 3 BC bất ổn địa bàn đồi núi khó đi (Kiến Đức, Quảng Tín, Đông Gia Nghĩa), cần theo dõi liên tục và chuẩn bị trước các phương án nhân sự, điều động kích thích năng suất từ những ngày đầu Event.")
...
add_centered_picture(doc, os.path.join(charts_dir, 'ghn_giao_tinh.png'), width_in_inches=6.2, caption_text="Biểu đồ 4: Sản lượng Giao theo Tỉnh/Quận & Tỷ trọng trung bình (Event 8.8 NTB)")
...
# SAVE MASTER DOCX
master_docx_paths = [
    r'C:\Users\lap4all\Downloads\NTB Kế hoạch Event 8.8_MUC_TIEU_FULL.docx',
...
for p in master_docx_paths:
    try:
        doc.save(p)
        print(f"Successfully saved Master Word Plan Document: {p}")
    except Exception as e:
        print(f"Could not save {p}: {e}")

print("Master Event 8.8 Plan Document generated successfully with explicit SECTION I. MỤC TIÊU!")
