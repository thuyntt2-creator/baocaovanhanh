...
backup_path = r"C:\Users\lap4all\Downloads\Bao_Cao_Quy_Hoach_ MANG LUOI NTB_FIXED.docx"
...
# 1. Open new / Split hubs (06 BC)
p = doc.add_paragraph()
r = p.add_run("1. Kế hoạch Mở mới / Tách Bưu cục & Tối ưu Mạng lưới (06 Bưu cục)")
r.bold = True
r.font.size = Pt(11)
...
items_iv1 = [
    ...
    ("6. Mở mới / Tách Bưu cục Nam Cam Ranh (Tỉnh Khánh Hòa): ", "Tách từ BC (KHO) Cam Linh cũ (ID: 22830000) để chia tải cho 6 xã/phường phía Nam (Ba Ngòi, Cam Bình, Cam Lập, Cam Phước Đông, Cam Thịnh Đông, Cam Thịnh Tây; Vol giao: 500 đơn/ngày, Vol lấy: 100 đơn/ngày, định biên 1 AM + 1 NVXL + 8 NVPTTT, chi phí CCDC & hạ tầng ban đầu 27.86 triệu đồng). BC Cam Linh cũ giữ phụ trách 6 phường trung tâm (Cam Linh, Cam Thuận, Cam Lợi, Cam Lộc, Cam Phú, Cam Phúc Nam; Vol giao: 700 đơn/ngày, Vol lấy: 100 đơn/ngày, định biên 1 AM + 1 NVXL + 9 NVPTTT).")
]
...
# 2. Overloaded & Relocation Hubs Table (14 BC)
p = doc.add_paragraph()
r = p.add_run("2. Danh sách Bưu cục Quá tải & Đề xuất Di dời / Mở rộng mặt bằng")
r.bold = True
r.font.size = Pt(11)
...
summary_items = [
    ("Bưu cục Mở mới / Tách bưu cục (06 BC): ", "BC Xuân Hương - Đà Lạt 2, BC Di Linh Hàng Vừa, BC Đông Hải, BC B'Lao Mới, BC Nam Thành, BC Nam Cam Ranh (tách từ Cam Linh)."),
...
print(f"Successfully saved updated DOCX to: {target_path}")
doc.save(backup_path)
print(f"Successfully saved backup DOCX to: {backup_path}")
