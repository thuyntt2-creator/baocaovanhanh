...
def inspect_dialog():
    ...
    with sync_playwright() as p:
        try:
            ...
            print("🖱️ Click mat-select format dropdown...")
            format_select.click()
            ghn_page.wait_for_timeout(1500)
            
            # Print all options visible
            print("\n--- OPTIONS IN FORMAT DROPDOWN ---")
            options = report_frame.locator('mat-option').all()
            if not options:
                # Options might be in ghn_page context overlay
                options = ghn_page.locator('mat-option').all()
            
            print(f"Found {len(options)} options:")
            for opt in options:
                print(f"  - mat-option: text='{opt.inner_text().strip()}'")
                
            # Click option with 'Excel'
            excel_opt = None
            for opt in options:
            ...
            if excel_opt:
                print(f"👉 Chọn Excel: '{excel_opt.inner_text().strip()}'")
                excel_opt.click()
                ghn_page.wait_for_timeout(1000)
            else:
                print("❌ Không tìm thấy option Excel trong dropdown.")
                
            # Take screenshot to verify Excel is selected
            ghn_page.screenshot(path="temp_excel_selected.png")
            print("📸 Đã chụp ảnh dialog và lưu tại: temp_excel_selected.png")
