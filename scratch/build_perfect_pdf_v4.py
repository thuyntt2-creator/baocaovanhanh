...
docx_out = r"C:\Users\lap4all\Downloads\Ban_Do_Quy_Hoach_NTB_Perfect_v4.docx"
...
def decorate_map_clean(src_path, dst_name, has_new=True, has_move=True):
    ...
    # White semi-transparent box with shadow
    ...
    print(f"Decorated map saved: {dst_name}")
    return dst_path
...
r_sub = p_sub.add_run("BẢN ĐỒ PHÂN VÙNG LÃNH THỔ & THẺ QUY HOẠCH (WHAT-IF) TRÍCH XUẤT TỪ HỆ THỐNG GÓC NHÌN MẠNG LƯỚI")
r_sub.bold = True
...
# Sections structure matching Telegram / Report format
sections = [
    {
    ...
    },
    {
        ...
        "items": [
            ...
            {
                "title": "2.2. Bản đồ Quy hoạch TP. Đà Lạt - BC Xuân Hương 2 Mở Mới",
                "img": os.path.join(artifact_dir, "image1.png"),
            ...
            {
                "title": "2.3. Bản đồ Quy hoạch Mở mới Bưu cục B'Lao Mới - TP. Bảo Lộc",
...
doc.save(docx_out)
print(f"Successfully created Final Perfect DOCX at: {docx_out}")
...
doc_com.Close()
word.Quit()
print(f"Successfully generated PERFECT PDF V4 at: {pdf_out}")
