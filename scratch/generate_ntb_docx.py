...
for p in doc_src.paragraphs:
    ...
    if is_heading:
        if current: parsed_items.append(current)
...
# Create clean Word document with 36 rows Table
...
headers = ['STT & ĐVHC Mới', 'Các Xã / Phường Cũ Sáp Nhập & BC Đang Phụ Trách', 'Bưu Cục Tiếp Nhận (Cover Mới)', 'Chi Tiết Điều Chuyển Tuyến & Phương Án']
widths = [Inches(1.8), Inches(2.5), Inches(1.5), Inches(2.2)]
...
for r_idx, item in enumerate(parsed_items):
...
out_docx = r'C:\Users\lap4all\Downloads\Thu_trinh_Quy_Hoach_Mang_Luoi_NTB_2026_Bang_Gop_Tuyen_Buu_Cuc.docx'
doc_out.save(out_docx)
print(f'Successfully generated docx route transfer table: {out_docx}')
