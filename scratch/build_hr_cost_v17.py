...
out_path = r'C:\Users\lap4all\Downloads\AOP_Hang_Nang_MAU_Vung_T7-T12_2026_final_v17.xlsx'

print("Đang tạo Báo cáo V17 (Thêm Mặt Bằng vào đáy bảng để khớp Total)...")
...
rent_fill = PatternFill(start_color="A6A6A6", end_color="A6A6A6", fill_type="solid")
...
# --- BLOCK 2: CHI PHÍ ---
sheet.cell(r, 1).value = "II. TỔNG CHI PHÍ THEO KHO/BC (VNĐ)"
sheet.cell(r, 1).font = red_font
r += 1
...
# [MỚI] Thêm dòng Mặt Bằng trước khi tính Tổng
sheet.cell(r, 1).value = "+ CHI PHÍ MẶT BẰNG CHUNG (Kéo từ Topline)"
sheet.cell(r, 1).font = bold_font; sheet.cell(r, 1).fill = rent_fill
for c_idx in range(6):
    c = sheet.cell(r, c_idx + 2)
    c.value = f"={cols[c_idx]}15" # Bằng đúng ô mặt bằng ở Topline
    c.font = bold_font; c.fill = rent_fill; c.number_format = money_format
r += 1

# Tổng cộng Block 2 (Đã bao gồm Mặt Bằng)
sheet.cell(r, 1).value = "TỔNG CỘNG CHI PHÍ TẤT CẢ"
sheet.cell(r, 1).font = bold_font; sheet.cell(r, 1).fill = total_fill
for c_idx in range(6):
    c = sheet.cell(r, c_idx + 2)
