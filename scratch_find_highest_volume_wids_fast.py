...
def main():
    ...
    for ws in sh.worksheets():
        title = ws.title
        if title == 'data thô':
            # We already know they exist in data thô, skip to save memory/time
            continue
        ...
        try:
            ...
            # Search for targets in df
            for tid in target_ids:
                # Find if any cell matches the target id
                mask = df.astype(str).apply(lambda col: col.str.strip() == tid)
                matches = df[mask.any(axis=1)]
                if len(matches) > 0:
                    print(f"  🎯 Found target '{tid}' in sheet '{title}'! {len(matches)} matching rows:")
