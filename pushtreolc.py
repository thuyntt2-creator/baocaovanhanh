...
# ============ CHẾ ĐỘ GỬI ============
# ENABLE_SEND = True  -> Gửi thật cho tất cả AM (mặc định)
# Truyền --dry-run từ CLI để xem trước mà không gửi
ENABLE_SEND = True
# =====================================
...
def open_sheet():
    ...
    if not oauth_path:
        raise FileNotFoundError("❌ Không tìm thấy authorized_user.json.")
...
def main():
    ...
    # Truyền --dry-run từ CLI để xem trước mà không gửi thật
    ...
    is_real_send = ENABLE_SEND and not is_dry_run
 
    if is_dry_run:
        print("👁️  [DRY-RUN MODE] Chỉ xem trước, KHÔNG gửi tin GTalk.\n")
    ...
    for i, (am_name, bc_dict) in enumerate(items, start=1):
        ...
        if not is_real_send:
            print(f"👀 [DRY-RUN] [{i}/{len(items)}] AM '{am_name}' ({total_nv} đơn - {len(messages)} tin nhắn) -> channel {channel_id}")
            for j, msg in enumerate(messages, 1):
                print(f"  --- Tin {j}/{len(messages)} ---")
