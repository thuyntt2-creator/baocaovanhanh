...
Yêu cầu cấu hình:
  - File `credentials.json` (Google Service Account) đặt cùng thư mục với script này
    (hoặc sửa hàm get_credentials_path bên dưới để trỏ đúng đường dẫn).
  - Phải SHARE (chia sẻ quyền chỉnh sửa) Google Sheet đích cho email của
    service account đó (nằm trong file credentials.json, key "client_email").
...
def upload_to_sheet(df, sheet_key, tab_name, gid=None):
    ...
    if not json_path:
        print("❌ Không tìm thấy credentials.json.", flush=True)
        return False
    try:
        scopes = [
